        ...
        <div class="panel-body">
          <div class="box-button">
            ...
            @if (Auth::check() && Auth::user()->level == 'admin') 
                  {{ link_to('/jadwalmagang/create', '', ['class' => 'btn btn-success btn-sm glyphicon glyphicon-plus']) }}
            @endif
              </div>
              ...
              <div class="col-md-12 table-responsive">
                ...
                <?php foreach ($jadwalmagang_list as $instansi): ?>
                    ...
                    @if (Auth::check() && Auth::user()->level == 'admin') 
                       <td>  
                        <div class="box-button">
                           {{ link_to('jadwalmagang/'. $instansi->id, '', ['class' => 'btn btn-primary btn-sm glyphicon glyphicon-th-list']) }}
                           {{ link_to('jadwalmagang/'. $instansi->id . '/edit', '', ['class' => 'btn btn-warning btn-sm glyphicon glyphicon-edit']) }}
                      </td>
                    </tr>
                    @endif    
                <?php endforeach ?> 
