...
<h4>Edit Tugas Magang</h4>
	{{ link_to('tugasku', 'Kembali', ['class' => 'btn btn-default btn-sm']) }}
	{!! Form::model($tugas, ['method' => 'PATCH', 'class' => 'form', 'action' => ['TugaskuController@update', $tugas->id]]) !!}
		<div class="form-group{{ $errors->has('id_user') ? ' has-error' : '' }}">
			{!! Form::label('id_user','Nama :', ['class' => 'control-label']) !!}
				@if(count($list_pengguna) > 0)
					{!! Form::select('id_user', $list_pengguna, null, ['class' => 'form-control', 'id' => 'id_user', 'disabled' => 'disabled', 'placeholder' =>'-- Pilih Nama Pengguna --']) !!}
				@else
		...
		<div class="form-group{{ $errors->has('tugas') ? ' has-error' : '' }}">
			...
			{!! Form::text('tugas',null, ['class' => 'form-control', 'disabled' => 'disabled']) !!}
			@if ($errors->has('tugas'))
		...
		<div class="form-group{{ $errors->has('deskripsi') ? ' has-error' : '' }}">
			...
			{!! Form::textarea('deskripsi',null, ['class' => 'form-control', 'disabled' => 'disabled']) !!}
		...
		<div class="form-group{{ $errors->has('tanggal_tugas') ? ' has-error' : '' }}">
			...
			{!! Form::date ('tanggal_tugas', !empty($tugas) ? $tugas->tanggal_tugas->format('Y-m-d'): null, ['class' => 'form-control', 'id' => 'tanggal_tugas', 'disabled' => 'disabled']) !!}
